...
{
    public partial class frmManualAddChildStock : Form
    {

        #region Variables

        private BL_MANUAL_CHILD_STOCK _blObj = null;
        private PL_MANUAL_CHILD_STOCK _plObj = null;
        ...
        public frmManualAddChildStock()
        {
            try
            {
                InitializeComponent();
                _blObj = new BL_MANUAL_CHILD_STOCK();
        ...
        private bool ValidateSationChildPart()
        {
            ...
            {
                Common common = new Common();
                DataTable dt = common.ValidateStationWithChildPart(cbStationNo.Text.Trim(),txtScanBarcode.Text.Trim());
                if (dt.Rows.Count > 0)
                {
            ...
            try
            {
                ...
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    if (!txtScanBarcode.Text.Trim().Contains("$"))
                    {
                        lblMsg.BackColor = Color.Red;
                        lblMsg.ForeColor = Color.Yellow;
                        lblMsg.Text = "Invalid Child Barcode";
                        return;
                    }
                    ...
                    _plObj = new PL_MANUAL_CHILD_STOCK();
                    _blObj = new BL_MANUAL_CHILD_STOCK();
                    _plObj.DbType = "C1";
                    _plObj.Station_No = cbStationNo.Text.Trim();
                    ...
                    {
                        if (dataTable.Rows[0][0].ToString().Split('~')[0] == "0")
                        {
                            GlobalVariable.mStoCustomFunction.setMessageBox(GlobalVariable.mSatoApps, "Child Stock Added Successfully!!", 1);
